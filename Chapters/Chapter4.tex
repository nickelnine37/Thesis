...
In this subsection, we seek to verify several basic properties of the CGM and SIM. First, we 
...
We have shown that the CGM will be preferable in the limit of a weak filter, when $\beta \rightarrow 0$, and that the SIM will be preferable in the limit of a strong filter, when $\beta \rightarrow \infty$. For filters that are neither very strong nor very weak, i.e. intermediate values of $\beta$, it is natural to ask which method can be expected to perform better. This decision will be more significant when $\gamma$ is small since, when it is large, both methods converge quickly.

For any fixed value of $m$, the SIM has a broad range of possible bounding convergence rates as a function of $\gamma$. The possible range for the CGM, by contrast, is restricted to a much narrower window. This behaviour is depicted explicitly in \cref{fig:conv_SIM_CGM_compared}. 

Given \cref{eq:n_SIM_SFL,eq:n_CGM_SFL}, under minor assumptions of continuity we should assume that higher values of $\beta$ monotonically decrease the number of steps required for convergence in both the SIM and CGM. To test this hypothesis, we ran a small experiment on synthetic data residing on a grid. In particular, we use a $16 \times 16$ grid of pixel data, and generate an observation matrix $\Y$ via white Gaussian noise and select 50\% of the pixels to be removed uniformly at random. We then count the total number of iterations required for convergence for both the SIM and the CGM over a range of values of $\gamma$. This was done for four values of $\beta$. We find that for higher values of $\beta$, the SIM achieves a better rate of convergence as $\gamma$ is decreased, whereas for low values of $\beta$, the CGM has superior scaling behaviour. These experiments confirm the results of the previous subsections. 
...
\begin{table*}[h]
    ...
    \toprule
    & \multicolumn{2}{c}{\textbf{SIM}} & \phantom{abc}& \multicolumn{2}{c}{\textbf{CGM}} & \phantom{abc} \\
