...
\begin{align}
    \M \vecc{\F_k} - \M \vecc{\F} &= \N \vecc{\F_{k-1}} + \vecc{\Y}  \notag \\
    \vecc{\E_k} &= \M^{-1} \N \vecc{\E_{k-1}} \notag \\
     &= \big( \M^{-1} \N \big)^{k} \vecc{\E_{0}}
\end{align}

where we denote $\vecc{\E_k} = \vecc{\F_k} - \vecc{\F}$. 

From this it is clear to see that convergence will be achieved so long as the spectral radius $\rho$ of the matrix $\M^{-1} \N$ is less than 1, since, if this condition holds then,

\begin{equation}
    \lim_{k \rightarrow \infty} \vecc{\E_{k+1}} = \lim_{k \rightarrow \infty} (\M^{-1} \N)^{k} \, \vecc{\E_1} = \mathbf{0} .
\end{equation}

Consider the magnitude of the error vector at iteration $k$. 

$$
e_k = || \, \big( \M^{-1} \N \big)^{k} \vecc{\E_0} \,||
$$

In the worst case scenario, where the initial error $\vecc{\E_0}$ is equal to the first eigenvector of $\M^{-1} \N$, which has eigenvalue $\rho$, it will be the case that
...
This implies that the number of iterations, $n$, required to reduce the magnitude of the error term to some small value $\epsilon$ is given by 
...
where we have used the shorthand $\U = \U_T \otimes \U_N$, $\D_\J = \Diag{\vecc{\J}}$ and $\D_{\Ss'} = \Diag{\vecc{\Ss'}}$. We would like to understand how to spectral radius $\rho(\M^{-1}\N)$ varies as a function of both the graph filter parameter(s) $\beta$ and the regularisation parameter $\gamma$. 

\vspace{0.8cm}

\textbf{Upper bound: the weak filter limit}

Consider the limiting case of a weak filter, where all spectral components are allowed to pass through unaffected. In this limit, the spectral scaling matrix $\G$ will contain all ones. Given \cref{eq:Jnt}, this further implies the every entry in $\J$ becomes $1 / (1 + \gamma)$. Given the definitions of the graph filters in \cref{tab:iso_filters,tab:anis_filters}, we can consider this as the limit where $\beta \rightarrow 0$. 


\begin{align}
    \lim_{\beta \rightarrow 0} \rho(\M^{-1}\N) &= \rho\Big(\U \D_\J \U^\top \D_{\Ss'} \Big) \notag \\
    &= \frac{1}{1 + \gamma} \rho\Big(\U \I \U^\top \D_{\Ss'} \Big) \notag \\
...
\textbf{Lower bound: the strong filter limit}
...
\begin{align*}
    \lim_{\beta \rightarrow \infty} \rho(\M^{-1}\N) &= \rho\Big(\U \D_\J \U^\top \D_{\Ss'} \Big) \\ 
    &= \frac{1}{1 + \gamma} \rho\Big(\U \mathbf{\Delta} \U^\top \D_{\Ss'} \Big)
\end{align*}
...
\textbf{Values of $\beta$ between 0 and $\infty$}
...
\begin{align}
\rho(\infty) &< \rho(\beta) < \rho(0)  \notag \\[0.1cm]
 \frac{1}{1 + \gamma} \, \frac{|\mathcal{S}'|}{NT} &< \rho(\beta) < \frac{1}{1 + \gamma}
\end{align}

The true dependence of $\rho$ on $\beta$ is likely to be complex since a variety of different filter functions are possible. But in general, we should expect that higher values of $\beta$ bring the true value of $\rho$ closer to the value given by the strong filter limit. If this is the case, then the fraction of observed nodes will also become relevant to the convergence rate. 


\vspace{0.8cm}


\textbf{A hard upper bound on complexity}



For a given requirement in error reduction, the complexity of \hyperlink{al1}{\textbf{algorithm 1}} is bounded by
