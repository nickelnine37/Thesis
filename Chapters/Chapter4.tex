...
\begin{equation}
    ...
    \vecc{\F_{k+1}} = \big(\HH^{-2} + \I \, \big)^{-1}  \Diag{\vecc{\Ss'}} \vecc{\F_{k}} + \big(\HH^{-2} + \I \, \big)^{-1} \vecc{\Y}
\end{equation}



Note that this splitting is valid since $\big(\HH^{-2} + \I \, \big)^{-1}$ is guaranteed to exist. It can also be readily computed as we already have the eigendecomposition of $\HH$. Noting the decomposed definition of $\HH$ given in \cref{eq:graph_filter}, this can be written as

\begin{align}
    \label{eq:inv}
    \M^{-1} & = \Big( \HH^{-2} + \I \, \Big)^{-1} \notag \\
            & = \Big( \big(\U_T \otimes \U_N\big)\, \Diag{\vecc{\G}}^{-1}\,  \big(\U_T^\top \otimes \U_N^\top\big)  + \I \, \Big)^{-1} \notag   \\
            & = \big(\U_T \otimes \U_N\big)\, \Big(  \Diag{\vecc{\G}}^{-1}\,    + \I \, \Big)^{-1} \big(\U_T^\top \otimes \U_N^\top\big) \notag \\
...
Since preconditioning of the coefficient matrix on the left is achieved with $\PHI^\top$ and on the right with $\PHI$, symmetry is preserved. This ensures that one can continue to utilise algorithms tailored to work with PSD matrices. In \hyperlink{al2}{\textbf{algorithm 2}}, we outline a conjugate gradient method based on this new formulation. Just as in section \ref{sec:SIM}, this process takes advantage of several identities involving Kronecker products and vectorization, to give $O(N^2T + NT^2)$ complexity per iteration. Note also that, in the case of a time-vertex problem where $\mathcal{G}_{T}$ is given by a path graph, this complexity can be further reduced to $O(N^2T + NT \log T)$. 
...
First, denote the difference between the true solution $\vecc{F} = $at the $k$-th iteration as $\E_k$, we can see that the error is updated according to the following equation.

\begin{align}
    \label{eq:E}
    \vecc{\E_{k}} & = \vecc{\F_{k}} - \vecc{\F_{k-1}} \notag  \\
    & = \M^{-1} \N \,  \vecc{\F_{k - 1}} + \M^{-1} \, \vecc{\Y} - \big(\M^{-1} \N \,  \vecc{\F_{k-2}} + \M^{-1} \, \vecc{\Y} \big)\notag \\
    & = \M^{-1} \N \big(\vecc{\F_{k-1}} - \vecc{\F_{k-2}} \big) \notag \\
    & = \M^{-1} \N \,  \vecc{\E_{k-1}} \notag \\
    & = \big(\M^{-1} \N\big)^{k-1} \,  \vecc{\E_1}
\end{align}
...
\begin{equation}
    \lim_{k \rightarrow \infty} \vecc{\E_{k+1}} = \lim_{k \rightarrow \infty} (\M^{-1} \N)^{k} \, \vecc{\E_1} = \mathbf{0} .
\end{equation}


For the stationary iterative method, equation (\ref{eq:E}) describes how the magnitude of the error, $\vecc{\E_k}$, is reduced at each iteration. Convergence is guaranteed as long as the spectral radius of $\M^{-1}\N$, denoted $\rho$, is less than one which we have demonstrated must be true. In particular, the error at iteration $k+1$ will take on the following value. 

\begin{align}
    \vecc{\E_{k+1}} &= \big(\M^{-1}\N\big)^{k} \vecc{\E_1} \notag \\ 
    &=  \big(\U \D_\J \U^\top \D_{\Ss'}\big)^{k} \vecc{\E_1}
\end{align}

where we have used the shorthand $\U = \U_T \otimes \U_N$, $\D_\J = \Diag{\vecc{\J}}$ and $\D_{\Ss'} = \Diag{\vecc{\Ss'}}$. The slowest possible rate of convergence will be achieved when $\vecc{\E_1}$ is proportional to the eigenvector of $\U \D_\J \U^\top \D_{\Ss'}$ with the largest associated eigenvalue, namely the spectral radius $\rho(\U \D_\J \U^\top \D_{\Ss'})$. In this case, we have that 

\begin{equation}
    \vecc{\E_{k+1}} = \rho(\U \D_\J \U^\top \D_{\Ss'})^{k} \vecc{\E_1}
\end{equation}

Consider the value of $\vecc{\E_1}$ that maximises the norm of the error for some value of $k$, subject to the constraint that $||\vecc{\E_1}||^2 = 1$. 

\begin{align}
    \mathbf{e}^\star &= \underset{\vecc{\E_1}}{\text{argmax}} \quad || \, \vecc{\E_{k+1}} \,||^2 \notag \\
    &= \underset{\vecc{\E_1}}{\text{argmax}} \quad \frac{\big(\M^{-1}\N \big)^{k} \vecc{\E_1}}{\vecc{\E_1}^\top \vecc{\E_1}}
\end{align}
