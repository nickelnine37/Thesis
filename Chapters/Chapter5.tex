...
\section{Kernel Graph Regression for green bond yield prediction}

In this section we conduct an analysis of the multiway Kernel Graph Regression algorithm developed in \cref{sec:kgr_dd} as applied to the problem of predicting green bond yields. Recently, green finance has emerged as an important asset class for funding the transition to a lower carbon economy \citep{Peters2022}. Green bonds provide a way for governments and municipalities to raise capital for large projects with specific climate or environmental sustainability purposes. In this case study, we consider the yield time series for a number of green bonds issued by US state authorities, and attempt to predict the missing values given external factors such as yields on US government treasuries. 

Bond yields over various maturities present an interesting use case for multiway graph signal processing methods. At any given time, for a given bond issuer, the yield over various maturities is expected to interpolate smoothly, forming the so called yield curve. Typically, shorter-maturity bonds have lower yields than those with a longer maturity, however under certain circumstances the trend may be flat or inverted. \Cref{fig:US_yield_curve} shows how the yield on US treasuries of various maturities has varied in the last 35 years.  
 
\begin{figure}[t] 
    \begin{center}
        \includegraphics[width=0.8\linewidth]{Figures/fed_rates.pdf}
    \end{center}
   \caption[3D plot of yield on US treasuries of various maturities over time]{A representation of the graph used for categorising municipal green bonds based on the sector of the project. } 
    \label{fig:US_yield_curve} 
\end{figure} 

For bonds issued by other bodies such as state authorities there may be many other intrinsic factors that affect the yield. These include the credit rating of the issuer, the coupon rate, the tax exemption status, and, of particular relevance to green bonds, the Environmental, Social and Governance (ESG) impact of the project being funded. In this case study, we frame the problem of estimating unknown yields as a multiway graph signal processing problem, where several of these factors are treated as separate tensor coordinates. In particular, we categorise each bond based on the following factors. 


\begin{enumerate}
    \item \textit{Use of funds}. Each bond in our dataset has an associated project category (such as water, pollution or power) and sub-category (such as waste management, habitat conservation, or solar farms). We used this information to construct a tree graph, beginning with a root node, and branching into category and sub-category nodes. The resultant tree, with a total of 161 nodes, is visualised in \cref{fig:green_bond_graph}. Each bond can then be associated with a particular leaf node based on their particular project. (No bonds were associated with the root or parent category nodes and, as such, all data here was interpreted as missing). 
    \item \textit{Credit rating}. Each bond also had an assigned credit rating associated with the issuer, designated by a ratings agency. These range from the highest possible rating of ``AAA", to the lowest in our dataset of ``BBB+", giving a total of eight categories.  
    \item \textit{Maturity}. Each bond has a fixed amount of time from its issue date to the date of maturity. Unlike treasury bonds, the green bonds we considered did not necessarily have standard maturity lengths. We therefore categorised each bond by the length of this period, sorted into seven different bins, with edges corresponding to 0, 1, 2, 5, 10, 20, 30, 40 and 50 years.  
    \item \textit{Coupon}. Each bond also had an associated coupon, expressed as a percentage of the principal, with biannual payments. We categorised each bond based on which of six bins this percentage fell within, using edges of 0, 1, 2, 3, 4, 5, and 6\%. 
\end{enumerate} 
...
Given these factors, each bond could be associated with a particular four dimensional coordinate. With the addition of a time coordinate, which included monthly data from March 2003 to March 2023, the resultant input tensor $\Yt$ had five distinct axes, with an overall shape of $(421, 161, 8, 8, 6)$. This can be understood as $T=421$ repeated measurements of a Cartesian product graph signal, where the first factor graph has an adjacency matrix specified by the tree shown in \cref{fig:green_bond_graph}, and the other three axes are simple chain graphs. \Cref{fig:bond_tensor} gives a visual representation of this object. 

\begin{figure}[t] 
    \begin{center}
        \includegraphics[width=0.85\linewidth]{Figures/bond_tensor.pdf}
    \end{center}
...
For many specific tensor coordinates, no bond yield existed. This could be due to the fact that the bond was not trading at this particular time, or because no bond ever existed with these particular characteristics. Furthermore, as mentioned, since the parent nodes in the graph representing use of funds are in a sense ``artificial'', in that they serve only to create the tree structure, no realised yield could possibly exist there. Under the framework developed in this thesis, this does not present a problem, since we can simply allocate these coordinates as containing missing data, as specified by the binary sensing tensor $\St$. 

Additionally, there were certain bonds that shared the same characteristics, i.e. the same use of funds, rating, maturity and coupon. In these cases, we simply chose the bond with the longest available history and broke ties randomly. In total, we used 1045 bonds, over an average of 56.2 months, covering a total of 0.4\% of the total tensor space. 

As mentioned, in this case study, we use Kernel Graph Regression to predict the yields. This means, in addition to the input tensor $\Yt$ and the sensing tensor $\St$, a matrix of explanatory variables $\X$ of shape $(T, M)$ is required. Here, we use the treasury yields over $11$ different maturities, along with core inflation and Federal reserve fund rate, to give a total of $M=13$ explanatory variables.  
